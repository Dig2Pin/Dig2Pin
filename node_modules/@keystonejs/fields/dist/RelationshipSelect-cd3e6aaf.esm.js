    ...
    MenuList: _ref => {
      ...
      useIntersectionObserver(([{
        ...
        if (!props.isLoading && isIntersecting && props.options.length < count) {
          fetchMore({
            query: gql`query RelationshipSelectMore($search: String!, $skip: Int!) {${refList.buildQuery(refList.gqlNames.listQueryName, `(first: ${subsequentItemsToLoad}, search: $search, skip: $skip)`)}}`,
            variables: {
              search,
...
const RelationshipSelect = ({
  ...
  const query = gql`query RelationshipSelect($search: String!, $skip: Int!) {${refList.buildQuery(refList.gqlNames.listQueryName, `(first: ${initalItemsToLoad}, search: $search, skip: $skip)`)}${refList.countQuery(`(search: $search)`)}}`;
